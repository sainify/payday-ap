...
import { Wallet, TrendingDown, PiggyBank, Receipt, ChevronRight, AlertCircle, Check } from "lucide-react";
  ...
  return (
    <div className="pb-28">
      ...
      <div className="px-5 space-y-4">
        {/* Hero: cycle ring + available balance */}
        <ClayCard className="flex items-center gap-5">
          <ProgressRing progress={progress} size={112} strokeWidth={10}>
            <div className="text-center">
              <div className="text-2xl font-display font-bold tabular">{cycle?.daysRemaining ?? "–"}</div>
              <div className="text-[10px] text-ink-faint uppercase tracking-wide">days left</div>
            </div>
          </ProgressRing>
          <div className="flex-1 min-w-0">
            ...
            <div className="text-sm text-ink-faint mt-2">
              ...
              <span className="text-ink dark:text-ink-inverted font-semibold">
                {cycle?.daysRemaining ?? "–"} day{cycle?.daysRemaining === 1 ? "" : "s"}
              </span>
            </div>
          </div>
        </ClayCard>
        ...
        <ClayCard className="bg-primary text-white !shadow-clay-raised">
          <div className="flex items-center justify-between">
            <div>
              <div className="text-white/80 text-sm mb-1">Safe to Spend Today</div>
        ...
        <div className="grid grid-cols-2 gap-4">
          <ClayCard padded className="!p-4">
            ...
            <div className="text-xs text-ink-faint mt-1">this salary cycle</div>
          </ClayCard>
          <ClayCard padded className="!p-4">
        ...
        <ClayCard>
          <div className="flex items-center justify-between mb-3">
            <h3 className="font-display font-semibold flex items-center gap-2">
              <Receipt size={18} /> Upcoming Bills
            </h3>
          ...
          {!loading && (data?.upcomingBills?.length ?? 0) === 0 && (
            <p className="text-sm text-ink-faint">No bills due soon. Nice and clear.</p>
          )}
              ...
              <div key={bill.id} className="flex items-center justify-between clay-inset px-4 py-3">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div className="text-xs text-ink-faint">Due {new Date(bill.due_date).toLocaleDateString("en-IN", { day: "numeric", month: "short" })}</div>
                  </div>
                </div>
        ...
        {/* Quick links */}
        <div className="grid grid-cols-2 gap-4">
          <Link to="/afford" className="clay-surface-sm clay-pressable p-4 block">
            <div className="font-semibold text-sm">Can I Afford It?</div>
            <div className="text-xs text-ink-faint mt-1">Check before you buy</div>
          </Link>
          <Link to="/splitter" className="clay-surface-sm clay-pressable p-4 block">
            <div className="font-semibold text-sm">Salary Splitter</div>
            <div className="text-xs text-ink-faint mt-1">Needs · Savings · More</div>
          </Link>
        </div>
      </div>
    </div>

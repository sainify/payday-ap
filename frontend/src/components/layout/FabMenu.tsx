...
import { TrendingDown, TrendingUp, Landmark, Receipt, Target } from "lucide-react";
      ...
      <div className="space-y-3">
        {options.map((opt) => (
          <button
            ...
            className="w-full clay-surface-sm clay-pressable flex items-center gap-4 p-4 text-left"
          >
            <div className={`h-12 w-12 rounded-clay-sm flex items-center justify-center ${opt.color}`}>
              <opt.icon size={22} />
            </div>
            <div>
              <div className="font-semibold">{opt.label}</div>
              <div className="text-sm text-ink-faint">{opt.sub}</div>
            </div>
          </button>
        ))}
      </div>

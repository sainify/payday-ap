...
export async function getDashboard(ctx: Ctx): Promise<Response> {
  ...
  const { predicted, avgDailySpend } = predictCycleEndBalance(availableBalance, spentThisCycle, cycle.cycleDay, cycle.totalDays);
  const knownFutureCommitments = upcomingBillsTotal + (recurringDue?.total || 0) + (debtsDue?.total || 0);
  const forecastEndBalance = predicted - knownFutureCommitments;

  const smartAlerts: Array<{ type: string; severity: "info" | "warning" | "danger"; title: string; message: string }> = [];
  for (const row of budgetRows as Array<{ limit_amount: number; category_name: string; spent: number }>) {
    ...
    if (pct >= 100) smartAlerts.push({ type: "budget", severity: "danger", title: `${row.category_name} budget exceeded`, message: `${Math.round(pct)}% used this salary cycle.` });
    else if (pct >= 80) smartAlerts.push({ type: "budget", severity: "warning", title: `${row.category_name} budget is nearly used`, message: `${Math.round(pct)}% used this salary cycle.` });
  }
  if (avgDailySpend > safeToSpendToday && safeToSpendToday > 0) {
    smartAlerts.push({ type: "pace", severity: "warning", title: "Spending pace is high", message: `Your average is ₹${Math.round(avgDailySpend).toLocaleString("en-IN")}/day vs ₹${Math.round(safeToSpendToday).toLocaleString("en-IN")} safe today.` });
  }
  if (forecastEndBalance < 0) {
    smartAlerts.push({ type: "forecast", severity: "danger", title: "Cycle-end forecast needs attention", message: `At the current pace and known commitments, you may run short by about ₹${Math.round(Math.abs(forecastEndBalance)).toLocaleString("en-IN")}.` });
  } else if (availableBalance > 0) {
    smartAlerts.push({ type: "forecast", severity: "info", title: "Cycle-end forecast", message: `About ₹${Math.round(forecastEndBalance).toLocaleString("en-IN")} may remain at the current pace.` });
  }
